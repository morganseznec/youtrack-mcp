...
- **27 MCP tools** covering issues (create, read, update, comment, link, tag, assign, close, time-track, attach files), knowledge base articles (CRUD + search), projects, users, groups, and saved searches. This matches and extends the capability surface of [JetBrains' official YouTrack MCP server](https://www.jetbrains.com/help/youtrack/server/model-context-protocol-server.html). See the [Tool reference](#tool-reference-for-developers) for the full list.
